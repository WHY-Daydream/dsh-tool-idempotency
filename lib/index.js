    ...
    const api = {
        ...
        release(name, argumentsValue, options) {
            ...
            return accepted
                ...
                : {
                    ok: false,
                    error: `idempotency key fingerprint mismatch or stale executionId (${GENERATION_MISMATCH}) — refusing to release a record owned by different arguments or an older execution (ABA); re-query to obtain the current executionId`,
                };
        },
        confirm(name, argumentsValue, result, options) {
            ...
            return accepted
                ...
                : {
                    ok: false,
                    error: `idempotency key fingerprint mismatch or stale executionId (${GENERATION_MISMATCH}) — refusing to confirm a record owned by different arguments or an older execution (ABA)`,
                };
        },
        invalidate(name, argumentsValue, options) {
            ...
            return accepted
                ...
                : {
                    ok: false,
                    error: `idempotency key fingerprint mismatch or stale executionId (${GENERATION_MISMATCH}) — refusing to invalidate a record owned by different arguments or an older execution (ABA)`,
                };
        },
    };
