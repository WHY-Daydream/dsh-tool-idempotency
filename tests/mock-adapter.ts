/**
 * Self-contained scripted LLM adapter for the E2E suite.
 *
 * Adapted from deepseek-harness `packages/core/agent-loop/tests/mock-adapter.ts`
 * (2026-09-07, C 阶段整改 P1: E2E 不得再相对导入 DSH monorepo 内部源码)。本副本
 * 只依赖 `@deepseek-ai/dsh-llm` 的**公开导出**（LlmAdapter / StreamChunk / CallId /
 * GenerateOptions 等），因此 e2e.spec.ts 不再需要越界 `.ts` 导入，可重新纳入
 * tsconfig 类型检查。行为与上游逐行保持一致，便于对照。
 */
